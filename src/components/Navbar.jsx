...
const Navbar = () => {
	...
	return (
		<>
			{/* Mobile */}
			<div className="flex items-center gap-4 relative top-3 lg:hidden">
				<div className="w-10 h-10 rounded-full flex justify-center items-center flex-shrink-0" id="MenuButton">
					<img src="./NavIcon.png" alt="Menu" className="w-6 h-6 cursor-pointer" onClick={toggleMenu} />
				</div>
				<div className={`flex-1 text-left ${isMenuOpen ? "hidden" : ""}`}>
					<div className="text-[0.7rem]">Hi, Pengguna smatec.in_k !</div>
					<div className="font-bold text-[1rem]">WELCOME</div>
				</div>
				<div className="w-10 h-10 rounded-full flex justify-center items-center flex-shrink-0" id="UserButton">
				...
				<div
					className={`fixed top-0 left-0 h-full w-64 bg-black/80 backdrop-blur-md border-r border-white/10 px-4 overflow-y-auto shadow-lg transform transition-transform duration-300 ease-in-out z-20 ${
